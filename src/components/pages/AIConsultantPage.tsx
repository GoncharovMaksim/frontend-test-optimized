...
import { trackAIConsultant } from "../../utils/analytics";
...
function validateTelegramOrPhone(input: string): { isValid: boolean; error?: string } {
  ...
  if (trimmed.startsWith('@')) {
    return { isValid: false, error: "Имя пользователя должно содержать от 5 до 32 символов (буквы, цифры, _)" };
  }

  if (trimmed.startsWith('+')) {
    return { isValid: false, error: "Формат: +7 и 10 цифр (например: +79001234567)" };
  }
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8">
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-foreground text-sm sm:text-base">Популярные вопросы</h2>
          </div>
        ...
        >
          ...
          {/* Input */}
          <div className={`border-t border-border p-3 sm:p-4 bg-background/50 transition-all duration-500 ${
            !isUnlocked ? 'blur-sm pointer-events-none' : ''
          }`}>
            <form onSubmit={handleSubmit} className="space-y-2">
              ...
              {inputValue.length > 0 && (
                <div className="flex justify-end">
                  <span className={`text-xs ${inputValue.length >= 300 ? 'text-red-500' : 'text-muted-foreground'}`}>
              ...
              >
                ...
                >
                  <div className="bg-secondary/50 rounded-2xl border border-border p-5 sm:p-8 shadow-2xl">
                    ...
                    <h3 className="text-foreground text-center mb-2 sm:mb-3" style={{ fontSize: "1.25rem" }}>
                    ...
                    <form onSubmit={handleUnlock} className="space-y-3 sm:space-y-4">
                      <div>
                        <label htmlFor="telegram" className="block text-foreground mb-2 text-sm sm:text-base">
                      ...
                      >
                        <Send className="w-4 h-4 sm:w-5 sm:h-5" />
                        <span className="text-sm sm:text-base">Начать общение</span>
                      </motion.button>

                      <p className="text-muted-foreground text-center text-xs sm:text-sm">
                        Мы используем ваш контакт только для связи по вашему запросу
                      </p>
                    </form>
                  </div>
                </motion.div>
