...
import { Globe, Palette, Bot, Workflow, ArrowRight, Sparkles } from "lucide-react";
...
const services = [
  {
    ...
    description: "Создаем сайты любой сложности: лендинги, корпоративные порталы, интернет-магазины и веб-приложения.",
  ...
  },
  {
    icon: Palette,
    title: "Дизайн UI/UX",
    description: "Проектируем удобные интерфейсы и создаем визуальную айдентику, которая выделяет ваш продукт.",
  ...
  },
  {
    icon: Bot,
    title: "AI-агенты",
    description: "Разрабатываем чат-ботов, голосовых ассистентов и AI-инструменты для автоматизации задач.",
  ...
  },
  {
    ...
    description: "Находим и внедряем софт для автоматизации бизнес-процессов, повышаем эффективность компании.",
...
function ServiceCard({ service, index, onClick }: { service: typeof services[0]; index: number; onClick?: () => void }) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: animConfig.duration, delay: index * 0.1, ease: [0.22, 1, 0.36, 1] }}
      onMouseMove={handleMouseMove}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={handleMouseLeave}
      onClick={onClick}
      style={animConfig.complexAnimations ? {
        rotateX,
        rotateY,
      ...
      >
        ...
        <div className="relative flex flex-col h-full">
          ...
          <div className="mb-6 relative">
            <div className={`relative w-16 h-16 rounded-2xl bg-gradient-to-br ${service.gradient} flex items-center justify-center backdrop-blur-sm border border-white/10 group-hover:border-white/20 transition-all duration-300`}>
              {/* Icon */}
          ...
          {/* Hover indicator */}
          <div className={`flex items-center gap-2 text-accent transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
        ...
        {/* Corner accent */}
        <div className={`absolute top-4 right-4 w-2 h-2 bg-accent rounded-full transition-all duration-300 ${isHovered ? 'scale-100' : 'scale-0'}`} />
      </motion.div>
    </motion.div>
  );
}
...
export function Services({ onNavigate }: ServicesProps) {
  ...
  return (
    <section id="services" className="relative py-20 sm:py-32 px-6 lg:px-8 overflow-hidden">
      {/* Background effects */}
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div 
          ...
          style={animConfig.complexAnimations ? { perspective: "1000px" } : undefined}
        >
          {services.map((service, index) => (
            <ServiceCard key={service.title} service={service} index={index} onClick={handleCardClick} />
          ))}
        </div>
      </div>
    </section>
